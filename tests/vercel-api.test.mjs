...
import appleRecommendation from "../api/recommendations/apple.js";
import grapeRecommendation from "../api/recommendations/grape.js";
...
test("Vercel grape endpoint returns demo data until a new key is configured", async () => {
  const originalKey = process.env.DATA_GO_KR_SERVICE_KEY;
  delete process.env.DATA_GO_KR_SERVICE_KEY;

  try {
    const response = await grapeRecommendation.fetch(
      new Request("https://example.test/api/recommendations/grape?date=2026-08-25"),
    );
    const payload = await response.json();

    assert.equal(response.status, 200);
    assert.equal(payload.fruit.id, "grape");
    assert.equal(payload.source.status, "demo");
    assert.equal(payload.recommendations.length, 3);
    assert.equal(response.headers.get("cache-control"), "no-store");
  } finally {
...
test("Vercel grape endpoint rejects non-GET requests", async () => {
  const response = await grapeRecommendation.fetch(
    new Request("https://example.test/api/recommendations/grape", { method: "POST" }),
  );
...
test("Vercel apple endpoint returns demo data until a new key is configured", async () => {
  const originalKey = process.env.DATA_GO_KR_SERVICE_KEY;
  delete process.env.DATA_GO_KR_SERVICE_KEY;

  try {
    const response = await appleRecommendation.fetch(
      new Request("https://example.test/api/recommendations/apple?date=2026-09-05"),
    );
    const payload = await response.json();

    assert.equal(response.status, 200);
    assert.equal(payload.fruit.id, "apple");
    assert.equal(payload.source.status, "demo");
    assert.equal(payload.recommendations.length, 3);
    assert.equal(response.headers.get("cache-control"), "no-store");
  } finally {
...
test("Vercel apple endpoint rejects non-GET requests", async () => {
  const response = await appleRecommendation.fetch(
    new Request("https://example.test/api/recommendations/apple", { method: "POST" }),
  );
